...
class DistCov():
    def __init__(self, v_data, v_field, delta_field, coord_system, dist_cov):
        self.RA           = v_data[0]
        self.DEC          = v_data[1]
        self.z_obs        = v_data[2]
        self.mu           = v_data[3]
    ...
    def p_r(self, mu_sys):
        mu0 = self.d_sigmad(mu_sys)
        r, V_r, delta = self.precomputed
        cartesian_pos_r = (np.expand_dims(self.r_hat.T, axis=1)*np.tile(np.expand_dims(r, axis=0),(1,1,3)))
        density_term = (1. + delta).T
        delta_mu = 5*np.log10(r) + 25 - mu0
        return r * r * jnp.exp(-0.5*(delta_mu/self.e_mu)**2) * density_term
